...
public class DistNeuralNetRunnerTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Class[] reg_classes = {Weight.class, Sample.class};
        conf.registerKryoClasses(reg_classes);

        JavaSparkContext sc = new JavaSparkContext(conf);

        System.out.println("Data Loading...");
        Sample[] train_sample = MnistLoader.loadIntoSamples("C:/Users/Jaehong/Downloads/mnist_train.txt", true);
        Sample[] test_sample = MnistLoader.loadIntoSamples("C:/Users/Jaehong/Downloads/mnist_test.txt", true);

        JavaRDD<Sample> train_data = sc.parallelize(Arrays.asList(train_sample)).cache();
        JavaRDD<Sample> test_data = sc.parallelize(Arrays.asList(test_sample)).cache();
        ...
        DistNeuralNetRunner driver = new DistNeuralNetRunner(sc, net).setIterations(numIteration)
